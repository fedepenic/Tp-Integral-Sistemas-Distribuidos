...
func main() {
	...
	outputMW, err := middleware.CreateExchangePublisherMiddleware(outputExchange, outputKeys, conn)
	if err != nil {
		log.Fatalf("[cleaner] connect to output exchange: %v", err)
	}
	defer outputMW.Close()

	accountsMW, err := middleware.CreateExchangePublisherMiddleware(accountsExchange, []string{}, conn)
	if err != nil {
